const {test,expect} = require("@playwright/test");
const POManager = require("../pageobjects/PO_manager");
const dataSet = require("../test-data/Data.json");

test("Placing Order with Coupon Applied", async ({page})=>
{
    const credentials = dataSet.login[0]

    const poManager = new POManager(page);
    const loginPage = poManager.getLoginPage();
    await loginPage.goTo();
    await loginPage.login(credentials.mail,credentials.password);

    const dashboard = poManager.getDashboard();
    await dashboard.addToCart(dataSet.productName);

    await poManager.goToCart();

    const cartPage = poManager.getCartPage();
    await cartPage.checkOut();

    const placeOrderPage = poManager.getPlaceOrderPage();
    await placeOrderPage.fillPersonalinfo(dataSet.cvv,dataSet.expiryYear,dataSet.name)
    await placeOrderPage.applyCoupon(dataSet.coupon)
    await placeOrderPage.fillShippinginfo(dataSet.fillcountry,dataSet.country)
    await placeOrderPage.placeOrder()

});

test("Placing Order without Coupon Applied", async ({page})=>
{
    const credentials = dataSet.login[0]

    const poManager = new POManager(page);
    const loginPage = poManager.getLoginPage();
    await loginPage.goTo();
    await loginPage.login(credentials.mail,credentials.password);

    const dashboard = poManager.getDashboard();
    await dashboard.addToCart(dataSet.productName);

    await poManager.goToCart();

    const cartPage = poManager.getCartPage();
    await cartPage.checkOut();

    const placeOrderPage = poManager.getPlaceOrderPage();
    await placeOrderPage.fillPersonalinfo(dataSet.cvv,dataSet.expiryYear,dataSet.name)
    await placeOrderPage.fillShippinginfo(dataSet.fillcountry,dataSet.country)
    await placeOrderPage.placeOrder()

});